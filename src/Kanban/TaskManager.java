...
public class TaskManager {
    ...
    public void deleteTask(String taskName) {
        for (int i = 0; i < taskCount; i++) {
            if (taskNames[i].equalsIgnoreCase(taskName)) {
                for (int m = i; m < taskCount - 1; m++) {
                    developers[m] = developers[m + 1];
                    taskNames[m] = taskNames[m + 1];
                    taskIDs[m] = taskIDs[m + 1];
                    taskDurations[m] = taskDurations[m + 1];
                    taskStatuses[m] = taskStatuses[m + 1];
                }
                taskCount--;
                return;
            }
